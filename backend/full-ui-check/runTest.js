...
function parseArgs() {
  ...
  // Supported:
  // - node runTest.js <seedUrl>            (defaults to queue)
  // - node runTest.js <seedUrl> queue
  // - node runTest.js <seedUrl> local
  // - node runTest.js --resume <runId>
  const out = {
    seedUrl: null,
    mode: 'queue',
    resume: false,
...
async function run(){
  ...
  if (args.resume) {
    if (!args.resumeRunId) {
      console.error('Usage: node runTest.js --resume <runId>');
      process.exit(1);
    }
  ...
  if (args.mode === 'queue') {
    ...
    if (!seedUrl) {
      console.error('Usage: node runTest.js <seedUrl> queue');
      process.exit(1);
    }
  ...
  if (!rawArg) {
    console.error('Usage: node runTest.js <url> OR node runTest.js <url1,url2,...>');
    process.exit(1);
  }

  // Support both:
  //  - node runTest.js https://example.com
  //  - node runTest.js url1,url2,url3
  //  - node runTest.js "[\"url1\",\"url2\"]" (array passed as string)
  let urls = [];
