...
module.exports = async (page, config) => {
  ...
  debugLog('ADD ISSUE:', type);
  }

  var httpStatus = config.httpStatus;
  if (httpStatus === 404) {
    addIssue('Page Not Found (404)', {
      severity: 'major',
  ...
  async function safeShot(fileName) {
    debugLog('Saving screenshot:', path.join(screenshotDir, fileName));
  ...
  try {
    if (!page.__qaListenersAdded) {
      var failedRequestUrls = new Set();
      var ignoredRequestUrlRe = /facebook|instagram|cptn|analytics|legitscript|googletagmanager|doubleclick|google-analytics|googleadservices|google\.com\/rmkt|googlesyndication|hotjar|clarity\.ms|segment\.io|sentry\.io|hubspot|fullstory|mouseflow|crazyegg|optimizely|datadog|newrelic/i;
      ...
      page.on('requestfailed', function(req) {
        try {
          ...
          var errText = failure && failure.errorText ? failure.errorText : '';
          // Cancelled navigations / superseded prefetches are not user-facing failures
...
async function detectAndClosePopups(page, issues = []) {
  ...
  debugLog('[POPUP] Starting popup scan...');
  ...
  try {
    ...
    debugLog(`[POPUP] ${popupInfo.length} popup(s) detected`);
    ...
    try {
      ...
      debugLog('[POPUP] Screenshot failed');
    }
    ...
    try {
      ...
      debugLog('[POPUP] Escape key sent');
...
for (const selector of closeSelectors) {
  try {
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        debugLog(`[POPUP] Clicked: ${selector}`);
  ...
  return [...elements].some(el => {
    ...
    if (stillExists) {
      debugLog('[POPUP] Popup still exists after cleanup');
    } else {
