/**
 * generatePdffull.js — Production QA Report Generator
 *
...
function reportDebug(...args) {
  if (REPORT_DEBUG) console.log.apply(console, args);
}
...
function toHtmlRelativePath(relPath) {
...
function getScreenshotThumbsForFolder(screenshotDir, screenshotBaseUrl) {
  ...
  function walk(dir) {
    ...
    for (const ent of entries) {
      ...
      if (st.isDirectory()) {
        walk(abs);
      } else if (st.isFile() && ent.toLowerCase().endsWith('.png')) {
        const rel = path.relative(screenshotDir, abs).replace(/\\/g, '/');
        const htmlRel = toHtmlRelativePath(rel);
        out.push({
          file: path.basename(ent),
          relPath: rel,
          thumbSrc: `${screenshotBaseUrl}/${htmlRel}`,
          fullSrc: `${screenshotBaseUrl}/${htmlRel}`
        });
      }
    }
  }
  ...
  // Keep existing return shape used by HTML template.
  return out.map(x => ({ file: x.file, thumbSrc: x.thumbSrc, fullSrc: x.fullSrc, relPath: x.relPath }));
}
...
module.exports = function generateReport({
  ...
  if (report.length) reportDebug(JSON.stringify(report[0], null, 2));
  ...
  reportDebug('Screenshot folder:', resolvedScreenshotFolder);
  reportDebug('Folder exists:', fs.existsSync(resolvedScreenshotFolder));
  if (fs.existsSync(resolvedScreenshotFolder)) {
    reportDebug('Files:', fs.readdirSync(resolvedScreenshotFolder));
  }
  const allScreenshotFiles = getScreenshotThumbsForFolder(resolvedScreenshotFolder,screenshotBaseUrl);
  const ssMetaMap = buildScreenshotMetadataMap(report);
  ...
  const pageCardsHtml = entries.map(e => {
    ...
    const shotThumbs = entryScreenshots
      // Only render screenshots we can actually map to a discovered file.
      // This avoids generating broken URLs like `screenshots/<filename>` when the real files live under:
      // `/reports/${runId}/screenshots/<relPath>/<filename>.png`.
      .map(fn => allScreenshotFiles.find(s => s.file === fn) || null)
      .filter(Boolean);
  ...
  reportDebug('=== Screenshot Metadata Map ===');
  reportDebug(JSON.stringify(ssMetaMap, null, 2));
  ...
  const galleryHtml = galleryItems.length > 0
    ? galleryItems.map((s, idx) => `
        <div class="thumb gallery-item" data-device="${escapeHtml(s.device)}" data-page="${escapeHtml(s.url || s.page)}"
             onclick="openGalleryViewer(${idx})" data-index="${idx}">
          <img src="${escapeHtml(s.thumbSrc)}" alt="${escapeHtml(s.file)}" loading="lazy" />
          <div class="t">${escapeHtml(s.file)}<br><small style="color:var(--text);">${escapeHtml(s.device)} • ${escapeHtml(shortUrlLabel(s.url))}</small></div>
        </div>`).join('')
    : '<div class="empty-state">No screenshots available.</div>';

  // ── Visual QA Findings HTML ────────────────────────────────────────────────
  // let visualFindingsHtml = '';
  // if (visualQAFindings.length > 0) {
  //   const findingRows = visualQAFindings.map(issue => {
  //     const severity = classifySeverity(issue);
  //     const label = issueLabel(issue);
  //     const category = classifyCategory(issue);
  ...
  //     return `
  //       <tr>
  //         <td>${escapeHtml(entry ? entry.url : '')}</td>
  ...
  //     </div>`;
  // } else {
  //   visualFindingsHtml = `<div class="card findings-card"><div class="empty-state">✅ No visual QA findings detected.</div></div>`;
  // }
  ...
  if (galleryItems.length > 0) {
    printShotsHtml = galleryItems.map(function(s, idx) {
      return '<div class="print-screenshot-page">' +
        ...
        '<div class="print-shot-meta">Page: ' + escapeHtml(s.url || s.page) + '</div>' +
  ...
  for (var i = 0; i < galleryData.length; i++) {
    ...
    window.openGalleryViewer = function(idx) {
      ...
      document.getElementById('viewerInfo').textContent =
        item.file + ' — ' + item.device + ' • ' + (item.url || item.page);
