...
const REPORTS_DIR = moduleReportsDir('full-ui-check');
const HTML_PATH = path.join(REPORTS_DIR, 'qa-report.html');
const TRACKER_PATH = path.join(REPORTS_DIR, 'job-tracker.json');
...
async function getReport(reportId) {
  ...
  if (reportId === 'latest-html') {
    ...
    return html
      ? { meta: { id: reportId, type: 'full-ui-html', hasHtml: true }, data: { format: 'html', available: true } }
      : { error: 'NOT_FOUND', message: 'No Full UI check HTML report found.' };
  }

  const runPath = path.join(REPORTS_DIR, path.basename(reportId), 'qaReport.json');
  const data = await safeReadJson(runPath);
  if (!data) return { error: 'NOT_FOUND', message: 'Full UI check run report not found' };

  const tracker = await safeReadJson(TRACKER_PATH);
...
async function getHtmlForReport(reportId) {
  ...
  const html = await safeReadText(HTML_PATH);
  if (!html) return { error: 'NOT_FOUND', message: 'No Full UI check HTML report. Run: node backend/full-ui-check/runFullSiteUI.js <url>' };
  return { html };
}
