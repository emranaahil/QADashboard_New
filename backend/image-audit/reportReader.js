...
const { REPORT_JSON } = require('./imageReportStorage');
...
async function getReport(reportId) {
  ...
  try {
    ...
    return { error: 'NOT_FOUND', message: 'Image audit report not found' };
  }
...
async function getLatestReport() {
  ...
  if (!reports.length) {
    return { error: 'NO_REPORTS', message: 'No image audit reports found. Run an audit first.' };
  }
