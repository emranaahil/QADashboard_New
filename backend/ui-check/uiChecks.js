...
/** Set QA_DEBUG=1 for verbose per-check logging */
const QA_DEBUG = process.env.QA_DEBUG === '1';
function debugLog() {
  if (QA_DEBUG) console.log.apply(console, arguments);
}
...
module.exports = async (page, config) => {
  ...
  function addIssue(type, opts) {
    ...
    debugLog('ADD ISSUE:', type, '(total', issues.length + ')');
  }
  ...
  async function safeShot(fileName) {
    debugLog('Saving screenshot:', fileName);
  ...
  try {
    if (!page.__qaListenersAdded) {
      ...
      page.on('requestfailed', function(req) {
        try {
          var url = req.url();
          // Third-party / tracking / noise — not actionable layout defects
          if (/facebook|instagram|cptn|analytics|legitscript|googletagmanager|doubleclick|google-analytics|hotjar|segment\.|sentry|clarity\.ms|fonts\.gstatic|fonts\.googleapis|cdn\.jsdelivr|cloudflareinsights|favicon|adservice|googlesyndication|scorecardresearch|newrelic|datadog|browser-intake|chrome-extension|webpack|sourcemap/i.test(url)) {
            return;
          }
  ...
  debugLog('Running visual defect checks...');
...
async function detectAndClosePopups(page, issues = []) {
  ...
  try {
    ...
    if (!popupInfo.length) {
      debugLog("✅ [POPUP] No popup detected");
      return false;
    }

    debugLog(`⚠️ [POPUP] ${popupInfo.length} popup(s) detected`);
    ...
    try {
      ...
      debugLog(`📸 [POPUP] Screenshot captured: ${popupShot}`);
      ...
      debugLog("⚠️ [POPUP] Screenshot failed");
    }
    ...
    try {
      ...
      debugLog("⌨️ [POPUP] Escape key sent");
...
for (const selector of closeSelectors) {
  ...
  return [...elements].some(el => {
    ...
    if (stillExists) {
      debugLog("⚠️ [POPUP] Popup still exists after cleanup");
    } else {
      debugLog("✅ [POPUP] Popup successfully closed");
    }
  ...
  debugLog("❌ [POPUP] Failed:", err.message);
}
  // ═════════════════════════════════════════════════════════════════════════
...
img.addEventListener('error', function done() {
  ...
  try {
    if (isCompareMode) {
      debugLog('🔍 Running content comparison...');
  ...
  try {
    ...
    if (isContactHyperlinkEnabled()) {
      ...
      for (let ci = 0; ci < contactIssues.length; ci++) {
        ...
        // Preserve structured fields on last issue for aggregation
  ...
  try {
    ...
    debugLog('Report written:', issues.length, 'issue(s)');
  } catch (err) {
    console.error('⚠️ Report write failed:', err.message);
  }

  debugLog('QA COMPLETE:', issues.length, 'visual defect(s)');
