class ErrorCheckApp {
  ...
  async init() {
    await AppShell.init(this.moduleId);
  ...
  async start() {
    ...
    const maxUrls = parseInt(document.getElementById('maxPages').value) || 100;
  ...
  async pollProgress() {
    try {
      ...
      if (p.currentUrl) document.getElementById('currentUrl').textContent = 'Checking: ' + p.currentUrl;
    } catch { /* ignore poll errors */ }
  }
}

document.addEventListener('DOMContentLoaded', () => {
  const app = new ErrorCheckApp();
  app.init();
});