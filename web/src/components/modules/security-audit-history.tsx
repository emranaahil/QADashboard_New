...
import { api, type SecurityAuditHistoryItem, type SecurityAuditHistoryResponse } from "@/lib/api";
...
const EMPTY_MESSAGES: Record<TestType, string> = {
  "single-page": "No Single Page security audit reports found",
  "full-website": "No Full Website security audit reports found",
};
...
type Props = {
  ...
  onSelectReport: (item: SecurityAuditHistoryItem) => void;
...
export function SecurityAuditHistoryPanel({
  testType,
  onTestTypeChange,
  onSelectReport,
  onReportDeleted,
  ...
  const [data, setData] = useState<SecurityAuditHistoryResponse | null>(null);
  ...
  const load = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    setError(null);
    try {
      const result = await api.getSecurityAuditHistory({
        type: testType,
  ...
  const subheading = useMemo(() => {
    if (!stats.total) return "Past security audit runs";
    return `${stats.total} saved security report${stats.total === 1 ? "" : "s"}`;
  }, [stats.total]);
  ...
  return (
    <Card className="history-container w-full rounded-[20px]">
      <CardContent className="flex flex-col gap-6 p-8">
        <div>
          <h2 className="text-lg font-bold leading-tight">Security Audit History</h2>
        ...
        {!loading && !error && !data?.grouped?.length && (
          <div className="rounded-[14px] border border-dashed border-border bg-background-elevated px-6 py-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {debouncedSearch
                ? emptyMessage
                : "Run an audit above to generate your first report. Completed runs appear here automatically."}
            </p>
          </div>
        )}
