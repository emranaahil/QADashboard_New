...
import { api, type UiTestingHistoryItem, type UiTestingHistoryResponse } from "@/lib/api";

type TestType = "single-page" | "full-website";

const HEADINGS: Record<TestType, string> = {
...
function computeStats(items: UiTestingHistoryItem[]) {
  let completed = 0;
  let failed = 0;
  let running = 0;
...
function formatReportTime(item: UiTestingHistoryItem) {
...
export function UiTestingHistoryPanel({
  testType,
  onTestTypeChange,
  onSelectReport,
  refreshKey = 0,
  ...
  const [data, setData] = useState<UiTestingHistoryResponse | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await api.getUiTestingHistory({
        type: testType,
  ...
  const subheading = useMemo(() => {
    const label = testType === "full-website" ? "full website" : "single page";
    if (!stats.total) return `Past ${label} test runs`;
    return `${stats.total} saved ${label} report${stats.total === 1 ? "" : "s"}`;
  }, [stats.total, testType]);
  ...
  return (
    <Card className="history-container w-full rounded-[20px]">
      <CardContent className="flex flex-col gap-6 p-8">
        <div>
          <h2 className="text-lg font-bold leading-tight">{HEADINGS[testType]}</h2>
